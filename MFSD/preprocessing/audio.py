...
def get_librosa_features(path: str) -> np.ndarray:
    ...
    S_filter = librosa.decompose.nn_filter(S_full, aggregate=np.median, metric="cosine", width=int(librosa.time_to_frames(0.2, sr=sr)))
...
def get_audio_duration() -> None:
    ...
    print(
        sum(librosa.core.get_duration(filename=os.path.join(AUDIOS_FOLDER, filename)) for filename in tqdm(filenames, desc="Computing the average duration of the audios"))
        / len(filenames)
    )
